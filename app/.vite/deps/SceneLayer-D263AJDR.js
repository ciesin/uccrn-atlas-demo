  ...
  toJSON(e5) {
    ...
    return r4 ? this.toArray().map((t8) => {
      ...
      if (!r4.equals(t8.spatialReference)) {
        if (!L(t8.spatialReference, r4)) return (_a2 = e5 == null ? void 0 : e5.messages) == null ? void 0 : _a2.push(new s("scenefilter:unsupported", "Scene filters with incompatible spatial references are not supported", { modification: this, spatialReference: e5.layer.spatialReference, context: e5 })), null;
        const s8 = new j2();
        sn(t8, s8, r4), t8 = s8;
      }
      const s7 = t8.toJSON(e5);
      return delete s7.spatialReference, s7;
    }).filter((e6) => null != e6) : ((_b = e5 == null ? void 0 : e5.messages) == null ? void 0 : _b.push(new s("scenefilter:unsupported", "Writing Scene filters without context layer is not supported", { modification: this, spatialReference: e5.layer.spatialReference, context: e5 })), this.toArray().map((r5) => r5.toJSON(e5)));
  }
...
p9 = c3 = r([a2("esri.layers.support.PolygonCollection")], p9);
var l4 = p9;
...
async function i3({ fieldName: i4, statisticsInfo: a8, errorContext: o2, fieldsIndex: r4, path: n5, customParameters: l5, apiKey: c4, signal: f3 }) {
  if (null == a8) throw new s2(`${o2}:no-cached-statistics`, "Cached statistics are not available for this layer");
  const d3 = r4.get(i4);
  if (null == d3) throw new s2(`${o2}:field-unexisting`, `Field '${i4}' does not exist on the layer`);
  const h3 = a8.find((t8) => t8.name === d3.name);
  if (null == h3) throw new s2(`${o2}:no-cached-statistics`, "Cached statistics for this attribute are not available");
...
async function t7(e5) {
  const { BlobReader: t8, ZipReader: r4, BlobWriter: n5 } = await import("./zipjs-wrapper-WU44PBUH.js"), a8 = [], i4 = new r4(new t8(e5));
...
var Fe = class extends a7(F(R(l2(b2(j3(t3(S2(e2(i(a4.ClonableMixin(f))))))))))) {
  ...
  async queryRelatedFeatures(e5, t8) {
    if (await this.load(), !this.associatedLayer) throw new s2("scenelayer:query-not-available", "SceneLayer queries are not available without an associated feature layer", { layer: this });
  ...
  async queryCachedFeature(e5, r4) {
    ...
    if (!i4 || 0 === i4.length) throw new s2("scenelayer:feature-not-in-cached-data", "Feature not found in cached data");
    const s7 = new b3();
  ...
  async _loadAssociatedLayerForQuery() {
    ...
    try {
      ...
      throw new s2("scenelayer:query-not-available", "SceneLayer associated feature layer could not be loaded", { layer: this, error: e5 });
    }
  ...
  async applyEdits(e5, t8) {
    const { applyEdits: r4 } = await import("./editingSupport-M7JYDPIK.js");
    let i4 = t8;
    await this.load();
    const s7 = this.associatedLayer;
    if (!s7) throw new s2(`${this.type}-layer:not-editable`, "Service is not editable");
    await s7.load();
    const { globalIdField: a8 } = s7, n5 = !!s7.infoFor3D, l5 = (i4 == null ? void 0 : i4.globalIdUsed) ?? true;
    if (n5 && null == a8) throw new s2(`${this.type}-layer:not-editable`, "Valid globalIdField expected on editable SceneLayer");
    if (n5 && !l5) throw new s2(`${this.type}-layer:globalid-required`, "globalIdUsed must not be false for SceneLayer editing as globalIds are required.");
  ...
  async uploadAssets(e5, t8) {
    if (await this.load(), null == this.associatedLayer) throw new s2(`${this.type}-layer:not-editable`, "Service is not editable");
  ...
  async convertMesh(e5, t8) {
    ...
    await this.load(), this.infoFor3D || r4(new s2("invalid:layer", "SceneLayer has no capability for mesh conversion"));
  ...
  validateLayer(e5) {
    if (e5.layerType && !Le.has(e5.layerType)) throw new s2("scenelayer:layer-type-not-supported", "SceneLayer does not support this layer type", { layerType: e5.layerType });
    ...
    function t8(e6, t9) {
      ...
      if (!r4) throw new s2("scenelayer:unsupported-normal-reference-frame", "Normal reference frame is invalid.");
      if (!i4) throw new s2("scenelayer:incompatible-normal-reference-frame", "Normal reference frame is incompatible with layer spatial reference.");
    }
...
//# sourceMappingURL=SceneLayer-D263AJDR.js.map
